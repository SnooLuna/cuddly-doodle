...
for (n in 10:15){
    subject <- read.csv(paste0("data//behavioural//subject-", n, ".csv"))
...
```{r}
ggplot(test_data, aes(x=diff, y=retrieval_t, fill=condition)) + 
  geom_bar(position=position_dodge(), stat="summary", fun="mean", linewidth = 0) + theme_minimal(base_family = "DeliciousUni") + scale_fill_manual(values = c("#79d8d4", "#fdbecc")) +
  geom_errorbar(stat = "summary", width=.2, position = position_dodge(.9)) + scale_color_manual(values = c("gray40", "gray40"), guide = 'none') + labs(title = "Retrieval Time during Test Trials per Condition per Block", fill = "Condition") + ylab("Retrieval Time") + xlab("Block") + theme(plot.title = element_text(hjust = 0.5),
    panel.grid.minor = element_line(
      linetype = "dashed",
...
```{r}
ggplot(test_data, aes(x=block, y=alpha, fill=condition)) + 
  geom_violin() + theme_minimal(base_family = "DeliciousUni") + scale_fill_manual(values = c("#79d8d4", "#fdbecc")) +
  scale_color_manual(values = c("gray40", "gray40"), guide = 'none') + labs(title = "Retrieval Time during Test Trials per Condition per Block", fill = "Condition") + ylab("Retrieval Time") + xlab("Block") + theme(plot.title = element_text(hjust = 0.5),
    panel.grid.minor = element_line(
      linetype = "dashed",
      linewidth = 0.15,
      color = "gray65"
...
```{r}
ggplot(test_data, aes(x=alpha, y=retrieval_t, colour=condition)) + 
  geom_point() + theme_minimal(base_family = "DeliciousUni") + scale_colour_manual(values = c("#79d8d4", "#fdbecc")) + labs(title = "Retrieval Time during Test Trials per Condition per Block", fill = "Condition") + ylab("Retrieval Time") + xlab("Block") + theme(plot.title = element_text(hjust = 0.5),
    panel.grid.minor = element_line(
      linetype = "dashed",
      linewidth = 0.15,
      color = "gray65"
...
```{r}
m5 <- gam(LRT ~ trial + (condition + rep + block)^2 + condition:trial:rep
          + s(alpha, bs='re') + s(swahili, bs='re') + s(subject, bs='re') + s(subject, accuracy, bs='re'), data=test_data)

summary(m5)
```

```{r}
m1 <- gam(accuracy ~ trial + (condition + rep + block)^4
          + s(swahili, bs='re') + s(subject, bs='re'), data=test_data)
